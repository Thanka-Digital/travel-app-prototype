import LocationCard from "@/components/card/LocationCard";
import Button from "@/components/form/button/Button";
import MaxWidthWrapper from "@/layout/wrapper/MaxWidthWrapper";
import { ChevronsRight } from "lucide-react";
import { useState } from "react";
import { useNavigate } from "react-router-dom";
import BackButton from "@/components/global/BackButton";
import { LocationInfo } from "@/utils/locationDetailsData";

export default function TripPlanStep4() {
  const navigate = useNavigate();
  const [locationId, setLocationId] = useState<String | null>();

  function handleClick(id: String) {
    if (id === locationId) {
      setLocationId(null);
      return;
    }
    setLocationId(id);
  }

  return (
    <main className=" h-screen bg-white">
      <BackButton />
      <MaxWidthWrapper>
        <div className=" flex flex-col gap-10 my-20 mb-8 bg-white">
          <p className="font-semibold text-2xl text-center">
            Select a location
          </p>
          <section className="flex flex-col gap-4 mb-20">
            {LocationInfo?.map((locationInfo) => (
              <div
                onClick={() => handleClick(locationInfo.id)}
                key={locationInfo.id}
                className={`${
                  locationId === locationInfo.id
                    ? "outline outline-primary rounded-xl outline-offset-2"
                    : ""
                }`}
              >
                <LocationCard
                  image={locationInfo.image}
                  link={locationInfo.link}
                  visitPlaceName={locationInfo.visitPlaceName}
                  locationOfVisitPlace={locationInfo.locationOfVisitPlace}
                  visitorsCount={locationInfo.visitorsCount}
                />
              </div>
            ))}
          </section>
        </div>

        <section className="flex w-full justify-between items-center fixed bottom-0 left-0 px-6 py-4 bg-white">
          <p className="text-neutral_gray font-medium">4 of 6 steps</p>
          <Button
            className="rounded-full text-white px-6 h-9"
            disabled={!locationId}
            onClick={() => {
              navigate("/trip-plan/steps-5");
            }}
          >
            Next <ChevronsRight />
          </Button>
        </section>
      </MaxWidthWrapper>
    </main>
  );
}
